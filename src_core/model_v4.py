...
class FPNBridge(nn.Module):
    def __init__(self, base_width):
        ...
        bw = base_width
        # Lateral 1x1 convs
        self.lat5 = nn.Sequential(nn.Conv2d(bw * 8, bw * 8, 1, bias=False), nn.BatchNorm2d(bw * 8))
        self.lat4 = nn.Sequential(nn.Conv2d(bw * 8, bw * 8, 1, bias=False), nn.BatchNorm2d(bw * 8))
        self.lat3 = nn.Sequential(nn.Conv2d(bw * 4, bw * 4, 1, bias=False), nn.BatchNorm2d(bw * 4))
        self.lat2 = nn.Sequential(nn.Conv2d(bw * 2, bw * 2, 1, bias=False), nn.BatchNorm2d(bw * 2))
        self.lat1 = nn.Sequential(nn.Conv2d(bw, bw, 1, bias=False), nn.BatchNorm2d(bw))
        # Channel adapters for top-down path
        self.adapt_4to3 = nn.Sequential(nn.Conv2d(bw * 8, bw * 4, 1, bias=False), nn.BatchNorm2d(bw * 4))
        self.adapt_3to2 = nn.Sequential(nn.Conv2d(bw * 4, bw * 2, 1, bias=False), nn.BatchNorm2d(bw * 2))
        self.adapt_2to1 = nn.Sequential(nn.Conv2d(bw * 2, bw, 1, bias=False), nn.BatchNorm2d(bw))
        # Smooth 3x3 convs
        self.smooth5 = nn.Sequential(nn.Conv2d(bw * 8, bw * 8, 3, padding=1, bias=False),
                                     nn.BatchNorm2d(bw * 8), nn.ReLU(inplace=True))
        self.smooth4 = nn.Sequential(nn.Conv2d(bw * 8, bw * 8, 3, padding=1, bias=False),
                                     nn.BatchNorm2d(bw * 8), nn.ReLU(inplace=True))
        self.smooth3 = nn.Sequential(nn.Conv2d(bw * 4, bw * 4, 3, padding=1, bias=False),
                                     nn.BatchNorm2d(bw * 4), nn.ReLU(inplace=True))
        self.smooth2 = nn.Sequential(nn.Conv2d(bw * 2, bw * 2, 3, padding=1, bias=False),
                                     nn.BatchNorm2d(bw * 2), nn.ReLU(inplace=True))
        self.smooth1 = nn.Sequential(nn.Conv2d(bw, bw, 3, padding=1, bias=False),
                                     nn.BatchNorm2d(bw), nn.ReLU(inplace=True))

    def forward(self, b1, b2, b3, b4, b5, b6):
        p5 = self.smooth5(self.lat5(b5) + F.interpolate(b6, size=b5.shape[2:],
                                                         mode='bilinear', align_corners=True))
        p4 = self.smooth4(self.lat4(b4) + F.interpolate(p5, size=b4.shape[2:],
                                                         mode='bilinear', align_corners=True))
        p3 = self.smooth3(self.lat3(b3) + F.interpolate(self.adapt_4to3(p4), size=b3.shape[2:],
                                                         mode='bilinear', align_corners=True))
        p2 = self.smooth2(self.lat2(b2) + F.interpolate(self.adapt_3to2(p3), size=b2.shape[2:],
                                                         mode='bilinear', align_corners=True))
        p1 = self.smooth1(self.lat1(b1) + F.interpolate(self.adapt_2to1(p2), size=b1.shape[2:],
                                                         mode='bilinear', align_corners=True))
        return p1, p2, p3, p4, p5, b6
